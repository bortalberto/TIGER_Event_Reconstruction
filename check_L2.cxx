...
void check_L2(TString iname="event.root", TString oname="check_L2.pdf"){
    auto file = new TFile(iname);
    auto tree = (TTree*)file->Get("tree");
    //auto newfile = new TFile(oname,"recreate");

    //TH2F* h_charge_sh_channel[28];
    TH1F* h_charge_sh_chip[28][2];
    TH1F* h_time_chip[28][2];


    auto h_FEB_label_channel = new TH2F("FEB_lable_channel","Hits in FEB_label:channel",65,0,64,28,16,44);
    ...
    tree->Draw("charge_SH:t_min_ttrigg>>charge_time","charge_SH>-10&&charge_SH<40&&t_min_ttrigg>-80&&t_min_ttrigg<80&&channel!=20&&layer==2");

    //for(int i=0; i<28; i++){
    //    TString t_name1 = Form("charge_sh_channel_%d",i);
    //    TString t_name2 = Form("FEB_label_%d",i+16);
    ...
    //    TString name1 = Form("FEB_label==%d&&charge_SH>-25&&charge_SH<25",i+16);
    ...
    for(int i=0; i<28; i++){
	for(int j=0; j<2; j++){
	    TString t_name = Form("FEB_label_%d_chip_%d",i+16,j+1);
	    h_charge_sh_chip[i][j] = new TH1F(t_name,t_name,100,-10,40);
	    TString name0 = Form("charge_SH>>FEB_label_%d_chip_%d",i+16,j+1);
	    TString name1 = Form("FEB_label==%d&&chip==%d&&charge_SH>-10&&charge_SH<40&&t_min_ttrigg>-80&&t_min_ttrigg<80&&channel!=20&&layer==2",i+16,j+1);
    ...
    for(int i=0; i<28; i++){
	for(int j=0; j<2; j++){
	    TString t_name = Form("time_FEB_label_%d_chip_%d",i+16,j+1);
	    h_time_chip[i][j] = new TH1F(t_name,t_name,80,-80,80);
	    TString name0 = Form("t_min_ttrigg>>time_FEB_label_%d_chip_%d",i+16,j+1);
	    TString name1 = Form("FEB_label==%d&&chip==%d&&charge_SH>-10&&charge_SH<40&&t_min_ttrigg>-80&&t_min_ttrigg<80&&channel!=20&&layer==2",i+16,j+1);
    ...
    tree->Draw("charge_SH>>h_charge","charge_SH>-10&&charge_SH<40&&t_min_ttrigg>-80&&t_min_ttrigg<80&&channel!=20&&layer==2");
    ...
    tree->Draw("t_min_ttrigg>>h_time","charge_SH>-10&&charge_SH<40&&t_min_ttrigg>-80&&t_min_ttrigg<80&&channel!=20&&layer==2");
    //tree->Draw("charge_SH>>h_charge","charge_SH>-10&&charge_SH<40&&channel!=20");
    ...
    auto can_charge_sh_chip1 = new TCanvas("FEB_label_16-29_chip","FEB_label_16-29_chip",1600,800);
    can_charge_sh_chip1->Divide(4,7);
    for(int i=0; i<28; i++){
    ...
    auto can_charge_sh_chip2 = new TCanvas("FEB_label_30-43_chip","FEB_label_30-43_chip",1600,800);
    can_charge_sh_chip2->Divide(4,7);
    for(int i=28; i<56; i++){
	can_charge_sh_chip2->cd(i-27);
	h_charge_sh_chip[i/2][i%2]->Draw();
    }
    ...
    auto can_time_chip1 = new TCanvas("time_FEB_label_16-29_chip","time_FEB_label_16-29_chip",1600,800);
    can_time_chip1->Divide(4,7);
    for(int i=0; i<28; i++){
    ...
    auto can_time_chip2 = new TCanvas("time_FEB_label_30-43_chip","time_FEB_label_30-43_chip",1600,800);
    can_time_chip2->Divide(4,7);
    for(int i=28; i<56; i++){
	can_time_chip2->cd(i-27);
	h_time_chip[i/2][i%2]->Draw();
    }
