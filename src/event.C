 ...
 //return;
  ...
  for(std::multimap<int, int>::iterator elem = mevt.begin(); elem != mevt.end(); elem++){
	...
	for(int i=0; i<nhits; i++){
      ...
      TP_test(vtcoarse_L1_TP,vtcoarse_L2_TP);    
      if(print_hits) {
	for(int i=0;i<nhits;i++){
	  cout<<"hits id: "<<i<<" ROC: "<<tgemroc[i]<<" channel: "<<tchannel[i]<<" time: "<<t_min_ttrigg[i]<<endl;
	}
	cout<<"L1 TP: "<<moda(vtcoarse_L1_TP)<<endl;
	cout<<"L2 TP: "<<moda(vtcoarse_L2_TP)<<endl;
...
void TP_cout(){
  ...
  for(int i=0;i<N_TIGER;i++){
    ...
    cat = Form("abs(TP_diff[%d])<2 && TP_count[%d]==1" , i, i);
    ...
    cout<<"TIGER chip \t"<<i<<" efficiency: \t"<<eff[i]<<"\t and time efficiency: \t"<<eff_T[i]<<" and time efficiency +- 1 clock: \t"<<eff_Tpm1[i]<<endl;
  }
  return;
}
